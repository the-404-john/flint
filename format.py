...
from parser import binding_power
...
class Formater:
    ...
    def enum_type(self, node: EnumType) -> None:
        self.push(f"enum {node.iden}")

    def struct_type(self, node: StructType) -> None:
        self.push(f"struct {node.iden}")

    def union_type(self, node: UnionType) -> None:
        self.push(f"union {node.iden}")
    ...
    def array_decl(self, node: ArrayDecl) -> None:
        self.indent()

        self.type_prefix(node.arr_type)
        self.push(f" {node.iden}")
        self.type_suffix(node.arr_type)
    ...
    def enum_decl(self, node: EnumDecl) -> None:
        ...
        if node.enum_type is None:
            self.push("enum")
        else:
            self.type(node.enum_type)
    ...
    def union_decl(self, node: UnionDecl) -> None:
        ...
        if node.union_type is None:
            self.push("union")
        else:
            self.type(node.union_type)
    ...
    def typedef_decl(self, node: TypedefDecl) -> None:
        ...
        self.push("typedef ")
        self.type_prefix(node.underlying_type)

        self.push(f" {node.alias_iden}")
        self.type_suffix(node.underlying_type)
    ...
    def msg(self, node: AssertStmt | PrintLnStmt) -> None:
        if node.str_expr is not None:
            self.push(",")
            self.expr(node.str_expr, 0)

        if len(node.arg_exprs) != 0:
            assert node.str_expr is not None
    ...
    def empty_stmt(self, node: EmptyStmt) -> None:
        self.indent()
        self.push(";\n")
    ...
    def swtich_stmt(self, node: SwitchStmt) -> None:
        self.indent()
